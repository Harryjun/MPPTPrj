...
void gui_show_ptchar(u16 x,u16 y,u16 xend,u16 yend,u8 offset,u16 color,u16 size,u8 chr,u8 mode)
{
	...
	{		   
	    ...
	    {   
			if(size==12)temp=asc2_1206[chr][pos];  //调用1206字体
			else if(size==16) temp=asc2_1608[chr][pos];		 	//调用1608字体
...
void gui_show_str_center(u16 x,u16 y,u16 width,u16 height,u16 offset,u16 color,u16 rimcolor,u8 size,u8 *str)
{
	...
	gui_show_ptstr(x+xoff,y+yoff,x+width,y+height,offset,rimcolor,size,str,2);	//先画边(用大号点画)
	gui_show_ptstr(x+xoff,y+yoff,x+width,y+height,offset,color,size,str,1);		//画字(用小点画)
}
